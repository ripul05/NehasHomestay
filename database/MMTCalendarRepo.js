...
async function getActiveMMTImportConnections() {
    const query = `
        SELECT
            id,
            external_listing_id,
...
async function getMMTImportConnection(externalListingId) {
    const query = `
        SELECT
            id,
            external_listing_id,
...
async function replaceExternalBookingRooms(externalBookingId, roomIds = []) {
    ...
    try {
        ...
        await client.query(
            `DELETE FROM external_booking_rooms
...
async function markMissingEventsCancelled(
    externalListingId,
    currentExternalEventUids = []
) {
